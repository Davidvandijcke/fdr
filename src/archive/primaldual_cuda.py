...
class PrimalDual:
    ...
    def allocateTensors(self):
        ...
        self.f = torch.as_tensor(self.h_img).view(self.h,self.w,self.nc,)
    ...
    def run(self):
        ...
        for iter in range(self.repeats):
            pbar.update(1)
            #start = time.time()
            self.parabola() # project onto parabola (set K)
            continue
            ...
            self.ubar = self.ubar.detach()
            self.p1 = self.p1.detach()
            self.p2 = self.p2.detach()
            self.p3 = self.p3.detach()
            self.s1 = self.s1.detach()
            self.s2 = self.s2.detach()
            self.mu1 = self.mu1.detach()  
            self.mu2 = self.mu2.detach()
            self.mubar1 = self.mubar1.detach()
            self.mubar2 = self.mubar2.detach()
    ...
    def parabola(self):
        ...
        # Calculate mu1sum and mu2sum using self.mu1 and self.mu2
        ...
        # for z in range(0, self.l): # TODO: can this be further improved?
        #     mu1sum[:,:,:,z] = self.mu1[:,:,:,self.k_indices[z]].sum(dim=-1)
        #     mu2sum[:,:,:,z] = self.mu2[:,:,:,self.k_indices[z]].sum(dim=-1)
        ...
        # Unbind the stacked tensors into a list of tensors
        #mu1sum, mu2sum = torch.unbind(mu1_stack, dim=-1), torch.unbind(mu2_stack, dim=-1)
        ...
        u1 = p1 + self.sigmap * (u1 + mu1sum)
        u2 = p2 + self.sigmap * (u2 + mu2sum)
        u3 = p3 + self.sigmap * u3
        ...
        self.p1 = u1.detach().clone()
        self.p2 = u2.detach().clone()
        self.p3 = u3.detach().clone()
        
        
        # Call self.on_parabola with the selected elements TODO: left off here
        self.on_parabola(u1[mask], u2[mask], u3[mask], img, self.lmbda, torch.arange(1, self.l+1, dtype=torch.int64).expand(mask.shape)[mask], mask, self.l)
        

        
        del u1, u2, u3, mu1sum, mu2sum, B, mask, img, k_indices
        gc.collect()


        
    def bound(self, x1, x2, lmbda, k, l, f):
        return 0.25 * (x1*x1 + x2*x2) - lmbda * (k / l - f)**2
    ...
    def on_parabola(self, x1, x2, x3, f, lmbda, k, j, l):
        ...
        # v[mask2] = 0
        v[mask3] = 2 * torch.sqrt(-b[mask3]) * torch.cos((1 / 3) * torch.acos(a[mask3] /  torch.pow(torch.sqrt(-b[mask3]), 3)))
        ...
        self.p1[j] = torch.where(norm == 0, 0, (v / (2+0.25) ) * x1 / norm)
        self.p2[j] = torch.where(norm == 0, 0, (v / (2+0.25) ) * x2 / norm)
        self.p3[j] = self.bound(self.p1.detach().clone()[j], self.p2.detach().clone()[j], lmbda, k, l, f)
    ...
    def mu(self):    # first vectorized version
        ...
        tau = 1 / (2 + (float)(self.proj/4))
        K = 0
        # t1 = torch.zeros_like(self.mu1)
        # t2 = torch.zeros_like(self.mu2)
        # for k1 in range(self.l):
        #     for k2 in range(k1,self.l):
        #         t1[:,:,:,K] = self.p1[:,:,:,k1:k2+1].sum(dim=-1)
        #         t2[:,:,:,K] = self.p2[:,:,:,k1:k2+1].sum(dim=-1)
        #         K+=1
        ...
        t1 = torch.stack([self.p1[:,:,:,k1:k2+1].sum(dim=-1) for k1, k2 in k1_k2_combinations], dim=-1)
        t2 = torch.stack([self.p2[:,:,:,k1:k2+1].sum(dim=-1) for k1, k2 in k1_k2_combinations], dim=-1)

        c1 = self.mu1.detach().clone()
        c2 = self.mu2.detach().clone()
        self.mu1 = c1+tau*(self.s1-t1)
        self.mu2 = c2+tau*(self.s2-t2)
        self.mubar1 = 2 * self.mu1 - c1
        self.mubar2 = 2 * self.mu2 - c2
        #print("---- \n mu1 ------ \n", self.mubar1.flatten().sum())
        
        del t1, t2, c1, c2, k1_k2_combinations, tau, K
    ...
    def clipping(self):    # first vectorized version
        ...
        D = self.u+self.tauu*(d1+d2+d3)
        torch.clamp(D, min=0, max=1, out = self.u)
...
print("setting up")
test = PrimalDual(i = "marylin.png", o = "result.png", parm = "parameters.png", repeats = 10000, gray = True, level = 16, lmbda = 1, nu = 0.1)
print("running")
