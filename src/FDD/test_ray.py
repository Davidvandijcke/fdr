...
if __name__ == "__main__":
    ...
    # check simulations
    df = pd.read_csv("/Users/davidvandijcke/Downloads/simulations_2d_sigma_0.01_jsize_0.07107502654596967.csv")

    # Group by 'alpha', 'N', and 'S' and calculate the mean 'Y_jumpsize'
    df['Y_jumpsize'] = df['Y_jumpsize'].abs()
    mean_jumpsize = df.groupby(['alpha', 'N', 'S']).agg({'Y_jumpsize' : 'mean', 
                                                'mse' : 'mean', 
                                                'jump_neg' : 'mean',
                                                'jump_pos' : 'mean'}).reset_index()
