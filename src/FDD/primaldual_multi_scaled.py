...
class PrimalDual(torch.nn.Module):
    ...
    # Original __init__ code moved here (with 'self.' removed)
        ...
        tauu = torch.tensor(  1.0 / torch.sqrt(12) * res, device=dev)  # torch.tensor(  1.0 / 6.0 * res, device=dev) # *res
        sigmap = torch.tensor( (1.0 / (torch.sqrt(12)))  * res, device=dev) # torch.tensor( (1.0 / (3.0 + l))  * res, device=dev) # *res
        sigmas = torch.tensor( (1.0 / (torch.sqrt(12)))  * res, device=dev) # torch.tensor(1.0, device=dev) 
        ...
        tau = torch.tensor( (1.0 / (torch.sqrt(12)))  * res, device=dev) # (1.0 / (2.0 + (proj/4.0)))  
    ...
    def forward_differences(self, ubar, D : int):
        ...
        for dim in range(int(D)):
            ...
            diff = torch.cat((torch.diff(ubar, dim=dim), zeros), dim=dim)  / (1 / ubar.shape[0]) # / (1 / ubar.shape[0])
            diffs.append(diff)
    ...
    def parabola(self, px, pt, ubar, mux, lmbda, l, f, k_indices : Dict[int, List[int]], dims : List[int], sigmap):
        ...
        ut = (torch.cat((torch.diff(ubar, dim=-1), \
                         torch.zeros_like(ubar[...,:1], dtype = torch.float32)), dim = -1)) / (1 / ubar.shape[-1])  #/ (1 / ubar.shape[-1])
    ...
    def l2projection(self, sx, mubarx, sigmas, nu):
        ...
        mask = (norm > (nu ) * sx.shape[1] ) # * sx.shape[1]
    ...
    def clipping(self, px, pt, u, tauu, dims : List[int], l):
        ...
        dt = (torch.cat((pt[...,:-1], torch.zeros(dims + [1], device = u.device)), dim=-1) - \
            torch.cat((torch.zeros(dims + [1], device = u.device), pt[...,:-1]), dim=-1))  / (1 / pt.shape[-1]) # / (1 / pt.shape[-1])
    ...
    def backward_differences(self, p, dims : int):
        ...
        for i in range(dims):
            ...
            diff = (before_cat - after_cat)  / (1 / p.shape[1])  # / (1 / p.shape[1])
            output.append(diff)
